...
class CircleImageBase extends NodeBase {
  ...
  /**
   * Draws a badge on the node.
   * @param {CanvasRenderingContext2D} ctx - The canvas rendering context.
   * @param {object} options - The node options.
   * @param {number} [size] - The size of the badge.
   * @param {number} [offset] - The offset of the badge from the node.
   */
  _drawBadge(ctx, options, size = 10, offset = 0) {
    const x = this.width + offset + this.left;
    const y = this.top - offset;
    const label = options.badge.label ?? "";
    ...
    ctx.strokeStyle = options.badge.color ?? "#E2413C";
    ctx.fillStyle = options.badge.background ?? "#FFFFFF";
    ctx.beginPath();
    ...
    ctx.fillStyle = options.badge.color ?? "#E2413C";
    ctx.fillText(label, x, y + ctx.lineWidth, size);
  }
}
